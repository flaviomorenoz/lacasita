			...
			if ($user)
			{
				...
				else
				{
					...
					if(!empty($email_template))
					{
						...
						$sub = ucwords(str_replace('_',' ','Restablecer Contraseña')).' '.$this->config->item('site_settings')->site_title;
			...
			else
			{
				...
				else
				{
					...
					if(!empty($email_template))
					{
						...
						{
							$content 	= str_replace("__ACCOUNT_ACTIVATOIN_LINK__", anchor(SITEURL2.'/auth/activate/'.$id.'/'.$activation_code.'/'.$data['refer_by_code'],'Activa tu cuenta'),$content);
						}
						else
						{
							$content 	= str_replace("__ACCOUNT_ACTIVATOIN_LINK__", anchor(SITEURL2.'/auth/activate/'.$id.'/'.$activation_code,'Activa tu cuenta'),$content);
						}							
