    	...
    	<div class="modal-content">
      		<div class="modal-body">
       			<div class="login-block">
                	<button type="button" class="close modal-close" data-dismiss="modal" onclick="cerrar_alerta();">&times;</button>
                	<div class="login-block-header text-center">
						<br><?php echo get_languageword('login');?>
                	</div>
					<?php echo $this->session->flashdata('message'); ?>
					<?php 
					 	$attributes = array('name'=>'login_form', 'id'=>'login_form', 'class'=>'form-horizontal');
					 	echo form_open(URL_AUTH_LOGIN,$attributes);
					?>
                    <div class="form-group">
					<?php 
					 	$val='';
					 	if(isset($_POST['login'])){ $val = set_value('email'); }
					 	$element = array('name'=>'email', 'type'=>'email', 'class'=>'form-control form-control-lg', 'placeholder'=>get_languageword('email'));
					...
					<?php 
						$val='';
						if(isset($_POST['login'])){ $val = set_value('password'); }
						$element = array('name'=>'password', 'type'=>'password', 'class'=>'form-control form-control-lg', 'placeholder'=>get_languageword('password'));
                    ...
                    <div class="form-group">
                        <a href="#" onclick="show_popup('<?php echo $signup_modal;?>')" class="sing-link" style="font-size: 14px; color:#666666"><?php echo get_languageword('sign_up');?></a>
                        <a href="#" onclick="show_popup('<?php echo $forgot_modal;?>')" class="forgot-password-link" style="color: #666666;"><?php echo get_languageword('forgot_password?');?></a>
                    </div>
                 	<?php echo form_close();?>	  
	                <div class="login-with-social" style="display: none">
	                   <span><?php echo get_languageword('or_login_through');?></span> 
					   <a href="<?php echo $aauthUrl;?>"><img src="<?php echo FRONT_IMAGES;?>sign-g.svg" class="sc-icn"></a>
					   <a href="<?php echo $authUrl;?>"><img src="<?php echo FRONT_IMAGES;?>sign-f.svg" class="sc-icn"></a>
	                </div>
            	</div>
      		</div>
    	</div>
    	...
    	<div class="modal-content">
      		<div class="modal-body">
       			<div class="login-block">
                	<button type="button" class="close modal-close" data-dismiss="modal" onclick="cerrar_alerta();" >&times;</button>
					<div class="login-block-header text-center">
						<br><?php echo get_languageword('register');?>
                	</div>
					<?php echo $this->session->flashdata('message'); ?>
				
                	<?php 
					 	$attributes = array('name'=>'register_form','id'=>'register_form','class'=>'crunch-change');
				    ...
				    <?php 
						...
						$element = array('name'=>'first_name', 'id'=>'first_name', 'type'=>'text', 'class'=>'form-control form-control-lg', 'placeholder'=>get_languageword('first_name') );
                    ...
                    <div class="form-group">
                        ...
                        <a href="#" onclick="show_popup('<?php echo $lgin_modal;?>')" class="sing-link" style="font-size: 14px; color:#666666" ><?php echo get_languageword('Sign_In');?></a>
                        <a href="<?php echo URL_TERMS_CONDITIONS;?>" class="forgot-password-link" style="color: #666666;"><?php echo get_languageword('terms_conditions');?></a>
                    </div>
  ...
  <div class="modal-dialog ">
    ...
    <div class="modal-content">
      <div class="modal-body">
       <div class="login-block">
                 <button type="button" class="close modal-close" data-dismiss="modal" onclick="cerrar_alerta();">&times;</button>
                <div class="login-block-header text-center"><?php echo get_languageword('forgot_password?');?></div>
               ...
               <?php 
					...
					<a href="#" onclick="show_popup('<?php echo $lgin_modal;?>')" class="forgot-password-link" style="font-size: 14px !important;color: #666666;" ><?php echo get_languageword('login');?></a>
					
                    <a href="#" onclick="show_popup('<?php echo $signup_modal;?>')" class="sing-link" style="font-size: 14px !important;color: #666666;"><?php echo get_languageword('sign_up');?></a>
...
<script type="text/javascript">
    ...
    $("body").delegate(".fade","click",function(event){
        $(".alert").hide();
    })
</script>
...
function get_pincode2() {
	...
	if (city_id > 0 && spl_id > 0) {
			...
			$.ajax({			 
			 ...
			 async: false,
			 cache: false,	
			 url: "<?php echo base_url();?>cart/get_pincode2",
			 data: '<?php echo $this->security->get_csrf_token_name();?>=<?php echo $this->security->get_csrf_hash();?>&city_id='+city_id+'&spl_id='+spl_id,
			 success: function(data) 
			 {	
				...
				if(data != '' && data != 0)
				{
					...
					total = parseFloat( $('#total_cost2').val() ) + parseFloat(json['delivery_fee'] );
					t1 = new Intl.NumberFormat("es-PE", {style: "currency", currency: "PEN"}).format(total);
					$("#total_show1").hide();
					$("#total_show2").show();
					...
					$('#delivery_fee').val(json['delivery_fee']);
					$('#delivery_fee2').text('S/.'+json['delivery_fee']);
					$('#delivery_from_time').val(json['delivery_from_time']);
					$('#delivery_to_time').val(json['delivery_to_time']);
					$('#delivery_time_units').val(json['delivery_time_units']);
