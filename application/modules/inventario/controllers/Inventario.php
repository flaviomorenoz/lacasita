...
class Inventario extends MY_Controller{
    ...
    function index(){
        if(isset($_SESSION["USUARIO"])){
            if(strlen($_SESSION["USUARIO"])>0){
                $this->data['content']  = "inventario/stock";
                $this->_render_page('inv_templo', $this->data);
            }else{
                sleep(2);
                //die("Acceso denegado");
                $this->data["nada"] = "";
    ...
    function rep_entradas(){
        ...
        if( !(isset($_SESSION["USUARIO"]) && strlen($_SESSION["USUARIO"])>0) ){
            ...
            if(isset($_POST["desde"])){
                ...
                $this->load->view("reporte1", $data);  // lo devuelve por Ajax
            }else{
                $data["titulo"] = "Reporte de Entradas ";
                $data['content'] = "rep_entradas";

               $this->_render_page('inv_templo', $data);
            }
        }
    }
    ...
    function rep_productos(){
        ...
        $this->data["content"] = "rep_producto1";
    ...
    function rep_recetas(){
        ...
        $this->load->model('receta/receta_model');
        $result = $this->receta_model->rep_recetas();        
        
        $this->data["content"] = "rep_receta1";
        $this->data["result"] = $result;
    ...
    function rep_recetas_json(){
        ...
        $this->data["result"] = $result;
        $this->data["titulo"] = "Lista de Recetas";
        $this->load->view("inventario/rep_receta1",$this->data);
    }
    ...
    function detalle(){ // para cargar la parte inferior de Pantalla Movimiento
        ...
        $data["result"] = $result;
        $data["query"]  = $query;
        $this->load->view("detalle",$data);  // lo llama por Ajax
    }
    ...
    function actualiza_movimientos(){
        ...
        //$data['content'] = "inventario/movimientos";
        //$this->_render_page('inv_templo', $data);
        $this->load->view("inventario/movimientos",$data);
    }
