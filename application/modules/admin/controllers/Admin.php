...
class Admin extends MY_Controller
{
     ...
     * @return array
     **/ 
    function producto_mayor_demanda(){
        // $this->load->db();
        // $result=array();
        // $years=array();
        $qurey1 = "select distinct item_id, item_name from ".TBL_PREFIX.TBL_ORDER_PRODUCTS."";
        $products = $this->db->query($qurey1)->result();
        $data = [];
        foreach ($products as $p) {
            $query2 = $this->base_model->get_query_result("select sum(final_cost) final_cost from cr_order_products where item_id='".$p->item_id."'");
            foreach ($query2 as $s) {
                $data[] = array(
                    'Producto' => $p->item_name,
                    'Precio'=> $s->final_cost 
                );
            }
        } 
     ...
     * @return array
     **/ 
    function cliente_mayor_demanda(){
        $query1 = "SELECT DISTINCT customer_name FROM cr_orders ";
        $customer = $this->db->query($query1)->result();
        $data = [];

        foreach ($customer as $c) {
            $query2 = "SELECT sum(total_cost) as total_cost FROM cr_orders where customer_name = '".$c->customer_name."' "; 
            $total = $this->db->query($query2)->result();

            foreach ($total as $pt) {
                $data[] = array(
                    'Cliente' => $c->customer_name,
                    'Pedido'=> $pt->total_cost
                );
            }
