        ...
        if ($this->ion_auth->logged_in()) {
            ...
            if ($this->ion_auth->is_member()) {
                if (!empty($this->cart->contents())) {
                    ...
                    redirect(URL_MENU, 'refresh');
                }
            }

            if ($this->ion_auth->is_kitchen_manager()) {
                redirect(URL_KITCHEN_MANAGER, 'refresh');
            }

            if ($this->ion_auth->is_delivery_manager()) {
                redirect(URL_DELIVERY_MANAGER, 'refresh');
            }
        } else {
        ...
        if (isset($_POST['login'])) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($this->ion_auth->login($this->input->post('email'), $this->input->post('password'))) {
                    ...
                    redirect(URL_AUTH_INDEX, 'refresh');
                } else {
    ...
    function register()
    {
        ...
        if (isset($_POST['register'])) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $id = $this->ion_auth_model->register($identity, $password, $email, $additional_data, $groups);
                
                if($id){
                    $msg .= get_languageword($this->ion_auth->messages());
                    $cSql = "select id from cr_users where email = '{$email}'";
                    ...
                    if($nId > 0){
                        $cSql = "update cr_users set active=1 where id = {$nId}";
                        //die($cSql);
                        $this->db->query($cSql);
