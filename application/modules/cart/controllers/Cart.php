...
class Cart extends MY_Controller{
    ...
    function save_order2(){
        ...
        $zipcode = 0; // not user loging
        $input_data = array();

        $input_data['delivery_fee']       = $this->input->post('delivery_fee'); 
        
        $input_data['house_no']         = $this->input->post('house_no');
        $input_data['street']           = $this->input->post('street');
        $input_data['landmark']         = $this->input->post('landmark');

        $input_data['locality']         = $this->input->post('locality_name');
        $input_data['city']             = $this->input->post('city_name');
        $input_data['city_id']          = $this->input->post('city_id');
        $input_data['pincode']          = $this->input->post('pincode');

        $total_cost += $input_data['delivery_fee'];
        ...
        $input_data['customer_name']    = $this->input->post('customer_name');
        ...
        if ($input_data['payment_type'] == 'cashCard') {
            ...
            $input_data['payable_amount'] = $this->input->post('payable_amount');
        }
    ...
    function order_success()
    {
        ...
        if ($this->session->userdata('order_data') && $this->session->userdata('is_valid_request')) {
            ...
            if ($input_data['payment_type'] == 'cash' || $input_data['payment_type'] == 'cashCard') {
                ...
                $input_data['paid_amount']       = ($this->input->post('mc_gross')*3.55);
                ...
                $this->prepare_flashmessage("Operación inválida", 1);
                redirect(URL_CART_INDEX);
            }
                ...
                if ($order_id > 0) {
                    ...
                    if (!empty($email_template)) {
                        ...
                        if ($this->session->userdata('email')) {
                            ...
                            $to     = $input_data['payer_email'];
                        }

                        
                        $sub     = $this->config->item('site_settings')->site_title . ' - ' . ' PEDIDO ';
                        ...
                        $content         = str_replace("__TOTAL_COST__", $this->config->item('site_settings')->currency_symbol.number_format($input_data['total_cost'], 2), $content);
                        
                        $cash1 = '';
                        if ($input_data['payment_type'] == 'cashCard') {
                            $cash1 = 'Pago contra entrega'.' - '. $input_data['payment_card'];
                        }else{
                            $cash1 = 'Pago en Online';
                        }

                        $content         = str_replace("__PAYMENT_MODE__", $cash1 , $content);

                        $content         = str_replace("__CUSTOMER_MESSAGE__", 'Pedido reservado con éxito', $content);
                    ...
                    $this->prepare_flashmessage("Pedido no guardado debido a algún problema técnico. Por favor, póngase en contacto con el administrador", 2);
                    redirect(URL_CART_INDEX);
                }
            } else {
                $this->prepare_flashmessage("Pedido no guardado debido a algún problema técnico. Por favor, póngase en contacto con el administrador", 2);
    ...
    function paypal_cancel()
    {
        ...
        $this->session->unset_userdata('order_data');
        $this->cart->destroy(); // Destroy all cart data
        $this->prepare_flashmessage('Has cancelado tu transacción', 2);
        redirect(URL_CART_INDEX);
    }
    ...
    function get_localities() 
    {
        ...
        // if ($this->ion_auth->logged_in()) {
            ...
            if ($city_id > 0 ) {
                ...
                if (!empty($records)) {
                    $options .= '<option value="">Seleccionar Distrito</option>';
                    ...
                    $options .= '<option value="">No hay Distrito</option>';
                }
    ...
    function get_pincode2() 
    {
        ...
        // if ($this->ion_auth->logged_in()) {
            ...
            if ($city_id > 0 && $spl_id > 0) {
                ...
                $record = $this->base_model->get_query_result("select * from ".TBL_PREFIX.TBL_SERVICE_PROVIDE_LOCATIONS." where service_provide_location_id=".$spl_id." and city_id=".$city_id." and status='Active' ");
                
                if (!empty($record)) {
                    $data1['pincode'] = $record[0]->pincode;
                    $data1['delivery_fee'] = $record[0]->delivery_fee;
                    $data1['delivery_from_time'] = $record[0]->delivery_from_time;
                    $data1['delivery_to_time'] = $record[0]->delivery_to_time;
                    $data1['delivery_time_units'] = $record[0]->delivery_time_units;
